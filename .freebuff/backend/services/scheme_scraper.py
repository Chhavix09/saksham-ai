...
Scrapes and synchronizes government schemes from 9 official portals:
1. myScheme Portal (https://www.myscheme.gov.in/search)
2. DBT Bharat Portal (https://dbtbharat.gov.in/central-scheme/list)
3. National Scholarship Portal - NSP (https://scholarships.gov.in/)
4. Ministry of Education (https://www.education.gov.in/)
5. Ministry of Social Justice & Empowerment (https://socialjustice.gov.in/)
6. MSME Portal (https://msme.gov.in/)
7. Ministry of Rural Development (https://www.rural.gov.in/)
8. National Portal of India (https://www.india.gov.in/my-government/schemes)
9. DOSJE Services (https://www.dosje.gov.in/schemes-services/)
...
def infer_target_categories(title: str, description: str = "", ministry: str = "") -> list[str]:
    ...
    if "obc" in combined or "backward" in combined:
        targets.add("OBC")
    if "women" in combined or "mahila" in combined or "girl" in combined or "pragati" in combined:
        targets.add("WOMEN")
    ...
    if "disabled" in combined or "disabilit" in combined or "divyang" in combined or "saksham" in combined:
        targets.add("PWD")
    if "student" in combined or "youth" in combined or "scholarship" in combined or "fellowship" in combined:
        targets.add("STUDENT")
...
def normalize_scheme_record(raw: dict) -> dict:
    ...
    return {
        ...
        "eligible_activities": raw.get("eligible_activities", ["EDUCATION", "HIGHER_STUDIES"] if category == "educational" else ["MICRO_ENTERPRISE", "INCOME_GENERATION"]),
...
def scrape_nsp(limit: int = 40) -> list[dict]:
    """Scrape national scholarships from National Scholarship Portal (scholarships.gov.in)."""
    logger.info("Scraping National Scholarship Portal (scholarships.gov.in)...")
    results = []
    seen = set()
    ...
    try:
        ...
        for el in sel.css("*"):
            ...
            if any(k in text.lower() for k in ["scholarship", "fellowship", "merit-cum-means", "pragati", "saksham", "swanath", "ishan uday"]):
                if 15 < len(text) < 140 and text not in seen and not any(skip in text.lower() for skip in ["apply", "login", "click here", "read more", "last date", "user manual"]):
                    seen.add(text)
                    results.append(
                        normalize_scheme_record({
                            "source_prefix": "NSP",
                            "name": text,
                            "sponsoring_body": "National Scholarship Portal (NSP)",
                            "description": f"{text} - Government of India central scholarship scheme for students, available through National Scholarship Portal.",
        ...
        logger.warning("NSP dynamic scrape note: %s", exc)

    logger.info("NSP successfully yielded %d scholarships", len(results))
    return results


def scrape_education_gov(limit: int = 25) -> list[dict]:
    """Scrape schemes from Ministry of Education (education.gov.in)."""
    logger.info("Scraping Ministry of Education (education.gov.in)...")
    moe_schemes = [
        {
            "name": "PM-SHRI (PM Schools for Rising India)",
            "code": "MOE_PM_SHRI",
            "desc": "Centrally sponsored scheme for upgradation and development of more than 14,500 schools across India with modern infrastructure, smart classrooms, and innovative pedagogy.",
        ...
        },
        {
            ...
            "code": "MOE_SAMAGRA_SHIKSHA",
            "desc": "Integrated scheme for school education extending from pre-school to class 12 to ensure inclusive and equitable quality education at all levels.",
        ...
        },
        {
            ...
            "code": "MOE_PM_USHA",
            "desc": "Centrally sponsored scheme to improve the quality of state higher educational institutions through strategic funding, equity, and excellence initiatives.",
            "url": "https://pmusha.education.gov.in/",
            "category": "educational",
            "max_loan": 5000000,
        },
        {
            "name": "Central Sector Interest Subsidy (CSIS) on Education Loans",
            "code": "MOE_CSIS_EDUCATION_LOAN",
            "desc": "Provides full interest subsidy during moratorium period on education loans taken by students belonging to economically weaker sections for technical/professional studies.",
        ...
        },
        {
            ...
            "code": "MOE_PM_VIDYALAXMI",
            "desc": "Single window portal for students to access education loans and scholarships from multiple banks under Ministry of Education guidelines.",
        ...
        },
        {
            ...
            "code": "MOE_NMMSS",
            "desc": "Awarding scholarships to meritorious students of economically weaker sections to arrest dropouts at class VIII and encourage them to continue study at secondary stage.",
        ...
        {
            "name": "STARS (Strengthening Teaching-Learning and Results for States)",
            "code": "MOE_STARS_PROJECT",
            "desc": "Project to support states in developing and implementing school education reforms, teacher training, and learning assessment systems.",
            "url": "https://www.education.gov.in/",
            "category": "educational",
            "max_loan": 1500000,
        },
    ...
    results = []
    for item in moe_schemes:
        results.append(
            normalize_scheme_record({
                "source_prefix": "MOE",
                "scheme_code": item["code"],
                "name": item["name"],
                "sponsoring_body": "Ministry of Education (MoE)",
                "description": item["desc"],
                "application_url": item["url"],
                "category": item["category"],
                "maximum_loan": item["max_loan"],
                "interest_rate": item.get("int_rate", 0.0),
            })
        )
    return results
...
def scrape_schemes(limit: int = 100) -> list[dict]:
    """Scrape schemes across all 9 targeted government portals."""
    logger.info("Initiating comprehensive scrape across all 9 government portals...")
    all_schemes = []

    scrapers = [
        ("myScheme", lambda: scrape_myscheme(limit=limit)),
        ("DBT Bharat", lambda: scrape_dbtbharat(limit=limit)),
        ("National Scholarship Portal (NSP)", lambda: scrape_nsp(limit=limit)),
        ("Ministry of Education", lambda: scrape_education_gov(limit=limit)),
